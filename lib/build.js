...
module.exports = function(grunt) {
    ...
    grunt.initConfig({
        ...
        "uglify": {
            ...
            jmed: {
                src: '../mass/common/jmed.js', //压缩源文件是之前合并的buildt.js文件  
                dest: '../mass/common/jmed.min.js' //压缩文件为built.min.js  
            }
        },
        //替换文中静态资源，并加上版本号
        replace: {
        ...
        compress: {
            zip: {
                options: {
                    archive: '<%= config.destDir %>/<%= config.fileName %>.zip'
                },
                files: [{
                    cwd: '<%= config.destDir %>/',
                    src: ['**'],
            ...
            for (var key in pageData) {
                ...
                cfg.requirejs[channel + '_' + key] = {
                    options: {
                        ...
                        "uglify2": cfg.requirejs.blade.options.uglify2,
    ...
    function md5Replace(config, channel) {
        ...
        var cfg = {},
            tmp = {},
            cfgStr = '<script type="text/javascript">';
    ...
    grunt.registerTask('zip', 'compress to zip', function(channel) {
        var fileName = channel;
        if (!channel) channel = 'app';
        if (channel == 'app') fileName = 'blade';
